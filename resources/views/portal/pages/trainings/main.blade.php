        ...
        <div class="w-[95%] flex flex-col p-8 rounded-lg bg-white items-start">
            ...
            <div class="w-full flex">
                 ...
                 <!-- title header  -->    
                        ...
                        <span class="text-gray-500 text-sm">Number of Trainings: {{$items-> count()}} </span>
                    </div>
            </div>
            ...
            <span class="text-md font-bold text-gray-400">To add a new certificate: Filing Application > Choose Training > Fill up the form > Submit and wait for approval.</span>
             ...
             <div class="w-full flex flex-col overflow-y-auto h-[300px]">
                ...
                <div class="w-full grid grid-cols-[85%_15%] hover:bg-gray-100 px-4 py-4  border-l-8 border-red-900 border-b-2 border-t-2">
                        <div class="flex flex-col leading-tight">
                            <h1 class="font-semibold text-gray-700"> <strong> {{$item->type}} Training </strong>  - {{$item->title}}</h1>
                            <span class="italic text-sm text-gray-500"> Conducted By: {{$item->organization}}</span>
                            <span class="italic text-sm text-gray-500"> Date of Training: {{$item->start_date}} to {{$item->end_date}}</span>
                            <div class="flex items-start">
                                <img src = "{{asset('images/icons/attachment.png')}}" class="w-[20px] h-[20px]"/> 
                                <a title="{{$item->attachment}}" class="hover:underline text-gray-600" href = "{{asset('storage/trainings/'.  Auth::user()-> id .  '/' . $item->id . '.' . explode('.', $item->attachment)[1])}}" target="_blank"> {{$item -> attachment}}</a>
                            </div>
                        </div>
